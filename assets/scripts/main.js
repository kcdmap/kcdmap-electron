...
import {textMarkers, markers, usr_markers} from "./data/markers";
import $ from "jquery";
import './map';
import './app';
...
let  tilesUrl = "assets/map/{z}_{x}_{y}.jpg";
let  maxNativeZoom = 5;
let  mapMinZoom = 1;
let  mapMaxZoom = 5;
...
for (let  i = 0; i < usr_markers.length; i++) {
  ...
  imarkers.level = (imarkers.level == undefined) ? "" : imarkers.level;
    req = '<p class="req" data-i18n="req">Gereksinimler:</p><ul class="ilist"><li><i class="'+imarkers.req+'"></i><span class="iname" data-i18n="'+imarkers.req+'">'+imarkers.req.replace(/_/gi, " ")+'</span><span class="ilevel '+imarkers.level+'" data-i18n="'+imarkers.level+'">'+imarkers.level.replace(/_/gi, " ")+'</span></li>';
  }
  let  ilist = "";
...
function initUserLayerGroup() {
  ...
  if (localStorage.mapUserMarkers !== undefined) {
    ...
    for (let  i = 0; i < storageMarkers.length; i++) {
      ...
      <button class="copymarkerurl"><span class="sharetext" data-i18n="copylink">'+ get.interactive.cp_link[activeLang] +'</span>\
      <span class="copiedmsg hide">Copied</span></button>\
			<button class="edit-marker" data-i18n="edit_marker">'+ get.interactive.edit_marker[activeLang] +'</button>\
			<div id="edit-dialog" class="hide">\
			<div class="chooseIcon" data-i18n="choose_icon">'+ get.interactive.add_icon_title[activeLang] +'</div>\
      ...
      popupcontent = popupcontent+'</select>\
			...
			<button class="cancel" data-i18n="cancel">'+ get.interactive.textarea.cancel[activeLang] +'</button>\
			<button class="save-marker" data-i18n="Save">'+ get.interactive.textarea.save[activeLang] +'</button>\
			</div>\
			<button class="remove-marker" data-i18n="remove_marker">'+ get.interactive.remove[activeLang] +'</button>\
			<div id="remove-dialog" class="hide">\
			<span class="remove-text" data-i18n="remove_text">'+ get.interactive.dialog.title[activeLang] +'</span>\
			<button class="yes" data-i18n="yes">'+ get.interactive.dialog.yes[activeLang] +'</button>\
			<button class="no" data-i18n="no">'+ get.interactive.dialog.no[activeLang] +'</button></div>';
...
function addMarkerText(lat,long) {
  ...
  let  message = '<div class="chooseIcon" data-i18n="choose_icon">'+ get.interactive.add_icon_title[activeLang] +'</div>\
  ...
  message = message+'</select><div class="markertitle" data-i18n="marker_title">'+ get.interactive.marker_title[activeLang] +'</div>\
  <input type="text" id="titleprev" name="title" value="Arrow">\
  <div class="markerdesc" data-i18n="marker_desc">'+ get.interactive.marker_description[activeLang] +'</div>\
  ...
  <button type="submit" class="send" data-i18n="add">'+ get.interactive.add[activeLang] +'</button>\
  </form>';
  // 
  let  ltn = {};
  ltn.lat = lat;
...
<div id="remove-dialog" class="hide">\
<span class="remove-text" data-i18n="remove_text">'+ get.interactive.dialog.title[activeLang] +'</span>\
<button class="yes" data-i18n="yes">'+ get.interactive.dialog.yes[activeLang] +'</button><button class="no" data-i18n="no">'+ get.interactive.dialog.no[activeLang] +'</button></div>';
...
map.on('click', function (e) {
  ...
  if (long < 0 || long > 4095 || lat < 0 || lat > 4095) {
   console.log("lat: "+lat+ "long: "+long);
    ...
    message = '<span class="coordsinfo">X: ' +long+ ' ' + 'Y: ' +lat+ '</span><br><button class="add-marker" data-i18n="add_marker" onclick="addMarkerText('+lat+','+long+')">'+get.interactive.add_marker_button[activeLang]+'</button>';
